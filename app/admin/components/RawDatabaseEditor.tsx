...
export function RawDatabaseEditor({ collection, documentId, onClose, onSuccess }: RawDatabaseEditorProps) {
  ...
  return (
    <div className="fixed inset-0 z-50 bg-black/80 flex justify-center items-center p-4 sm:p-6 backdrop-blur-sm">
      <div className="bg-gray-900 w-full max-w-4xl p-4 sm:p-6 rounded-2xl shadow-2xl flex flex-col max-h-[90vh] border border-white/10">
        ...
        {loading ? (
          ...
          <div className="flex-1 flex justify-center items-center text-red-400 min-h-[300px]">
            {error}
          </div>
        ) : (
          <>
            <div className="flex-1 bg-black rounded-xl border border-white/10 p-2 overflow-hidden flex flex-col min-h-[400px]">
              <textarea
                value={rawJson}
            ...
            <div className="mt-4 flex justify-between items-center bg-yellow-500/10 border border-yellow-500/20 p-3 rounded-xl">
